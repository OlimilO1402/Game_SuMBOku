...
Private mImL1        As ImageList
Private WithEvents mTxtSudoku As DynTextBoxes
Attribute mTxtSudoku.VB_VarHelpID = -1
...
Private Sub RndExample_Click()
    Dim StrG As String: StrG = mRandomGames.GetNextGame
...
Private Sub SetGameTitle(Name As String)
    Dim StrCap As String: StrCap = "SuMBOku 1.2"
    If Len(Name) > 0 Then
        StrCap = StrCap & " - " & Name
    End If
    Me.Caption = StrCap
...
Private Sub Form_Load()
    ...
    mn = 3
    Set mTxtSudoku = New_DynTextBoxes(Me)
  
    'Set mSudoku = New SudokuGame
    'Set mUndoRedo = New SGUndoRedo
    Set mImL1 = New ImageList
    Call InitImL1
    'Call mSudoku.NewC(mn * mn)
    Set mSudoku = New_SudokuGame(mn * mn)
    'Set mUndoRedo.Sudoku = mSudoku
    Set mUndoRedo = New_SGUndoRedo(mSudoku)
    ...
    PrepareImgs
End Sub
Private Sub InitImL1()
    Dim i As Long
    'Voreinstellungen setzen
    mImL1.ColorDepth = Depth24Bit 'Depth8Bit ' Depth8Bit ' Depth16Bit ' ' '
    mImL1.TransparentColor = RGB(192, 192, 192) '&HC0C0C0
    mImL1.ImageSizeHeight = 24
    mImL1.ImageSizeWidth = 24
   'Images aus der Ressource laden
    For i = 1 To 2 '49
        Call mImL1.Images.Add(FromResource(, CStr(10 + i)), RGB(192, 192, 192), CStr(i))
    Next
End Sub
...
Public Sub SetupGame()
    'die Verbindung zwischen Textboxen und SudokuGame herstellen
...
Dim im As VB.PictureBox 'Image
  ...
  For i = 1 To index
    ...
    im.Cls
    'Call mImL1.Draw(im.hDC, 1, 1, , , 1)
    Call mImL1.Images.Item(2).DrawImage(im.hDC, 0, 0)
  Next
  'SunDark zeichnen
  For i = index + 1 To n
    Set im = Me.Controls("ImGL" & CStr(i))
    im.Cls
    'Call mImL1.Draw(im.hDC, 1, 1, , , 0)
    Call mImL1.Images.Item(1).DrawImage(im.hDC, 0, 0)
  Next
End Sub
...
Dim OFD As New OpenFileDialog
  ...
  If OFD.ShowDialog(Me) = DialogResult_OK Then
...
Dim SFD As New SaveFileDialog
  ...
  If SFD.ShowDialog(Me) = DialogResult_OK Then
...
Private Sub TxtSudoku_KeyPress(index As Integer, KeyAscii As Integer)
  ...
  'Notebooksupport ohne umschalten auf Nummernblock
  ...
  'für Sudoku mit bis zu 16 "Ziffern"
  Case "A", "B", "C", "D", "E", "F", "G"
  Case Else
